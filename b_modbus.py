...
def handle_verify_machine(machine_id):
  # if occupied
  if read_modbus_coil(SOcupy[machine_id]):
    return False
  # if free
  return True
...
def handle_destino(rotative_id, destino_array):
  ...
  # if not pass it to the next cell
  else:
...
def handle_request(modbus_user, cell, first_machine_used, second_machine_used, px, py, object_entries):
  ...
  if (handle_verify_machine(first_machine) and first_machine_used) or (handle_verify_machine(second_machine) and second_machine_used):
    ...
    if not h == True:
      print('machine reservation failed')
      return False

    print('machines reserved')
    ...
    if h == True:
      print('destino sucessfull')
    else: 
      print('destino failed')
...
th2 = threading.Thread(target=handle_request, args=(modbus_user, 0, True, False, 1, 3, [0]))
th2.start()
...
th4 = threading.Thread(target=handle_request, args=(modbus_user, 0, True, False, 1, 3, [0]))
th4.start()
